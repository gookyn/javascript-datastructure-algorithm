...
const { HashTableSeparateChaining } = require('../../../src/data-structures');
const { CustomObj } = require('../../utils');

describe('Hash Table Separate Chaining', () => {
	let hashTable;
	...
	it('put values with collisions', () => {
		...
		for (let i = 1; i <= 5; i++) {
			...
			let linkedNode = linkedList.getHead();
			expect(linkedNode.element.key).to.be.equal(i);
			expect(linkedNode.element.value).to.be.equal(i);

			linkedNode = linkedNode.next;
			expect(linkedNode.element.key).to.be.equal(i);
			expect(linkedNode.element.value).to.be.equal(i + 10);

			linkedNode = linkedNode.next;
			expect(linkedNode.element.key).to.be.equal(i);
			expect(linkedNode.element.value).to.be.equal(i + 100);
		}
	});
	...
	it('return string with collisions', () => {
		...
		hashTable.put('A', 'bb');
		expect(hashTable.toString()).to.equal('{28 => [#(A):aa],[#(A):bb]}');
	});
});
